...
The public API is `GET /api/billing/config`, `GET /api/guilds/:guildId/billing`, `POST /api/guilds/:guildId/billing/checkout`, `POST /api/guilds/:guildId/billing/cancel`, and `POST /api/billing/webhook/paypal`. Checkout accepts `plan` (`pro` or `ultimate`) and `method` (`paypal`, `card`, or `google_pay`). A method is disabled unless the required provider settings and PayPal plan IDs are present.
