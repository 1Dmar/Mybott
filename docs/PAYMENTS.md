...
The browser never grants an entitlement. It receives a provider approval URL, and the subscription becomes active only after a verified PayPal webhook is accepted by the backend. Webhooks are checked using PayPal's transmission headers and the provider's `verify-webhook-signature` endpoint. Event IDs are stored in `BillingEvent` with a unique provider/event index, so duplicate events do not repeat state transitions. `APPROVAL_PENDING`, `APPROVED`, or any `active`/`trialing` subscription without `metadata.paymentVerified === true` is treated as `payment_pending` and receives Free entitlements after refresh. Only a completed payment event can establish that proof.
...
The cancel endpoint also returns a safe PayPal/provider explanation and optional debug ID when the provider rejects cancellation. The implementation is **not live-provider tested** in this repository because production credentials were not supplied. Sandbox testing should use PayPal sandbox credentials and sandbox plan IDs, then verify checkout, webhook, duplicate event, cancellation, renewal, failure, and expiration scenarios before production activation. Raw card numbers are never stored or accepted by this application.
