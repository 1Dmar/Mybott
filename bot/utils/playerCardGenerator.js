    ...
    try {
        ...
        if (serverConfig && serverConfig.apiToken) {
            ...
            try {
                ...
                if (response.data && response.data.success) {
                    ...
                    // إذا كان firstPlayed = 0 و lastPlayed = 0 = لم يدخل السيرفر
                    const neverJoinedServer = (data.firstPlayed === 0 || data.firstPlayed === undefined) && 
                                            (data.lastPlayed === 0 || data.lastPlayed === undefined);
                    
                    // التفريق بين حساب مايكروسوفت أصلي وحساب مكركة (cracked)
                    // إذا كان لديه UUID حقيقي = أصلي، وإلا = مكركة
                    const isCrackedAccount = !data.uuid || data.uuid.includes("offline");
                    ...
                    return {
                        ...
                        customApi: true,
                        hasJoinedBefore: !neverJoinedServer,
                        neverJoinedServer: neverJoinedServer,
                        isCracked: isCrackedAccount,
        ...
        try {
            ...
            return {
                ...
                customApi: false,
                isCracked: false, // حساب من Mojang = أصلي
                skinUrl: `https://skins.mcstats.com/bust/${uuidResponse.data.name}`
            };
        } catch (mojangError) {
            console.warn(`Mojang API failed for ${ign}:`, mojangError.message);
            // إذا فشل Mojang، ربما يكون حساب مكركة
            return null;
        }
    } catch (error) {
...
async function generatePlayerCard(ign, template = 'glass', serverConfig = null) {
    ...
    // معالجة الحالات المختلفة للاعب
    if (!playerData) {
        // خطأ في الاتصال بالـ API أو Mojang
        ...
        ctx.textAlign = 'center';
        ctx.fillText('⚠ Player Not Found', width / 2, height / 2 - 20);
        
        ctx.fillStyle = 'rgba(255, 255, 255, 0.6)';
        ctx.font = '20px Arial';
        ctx.fillText('Username does not exist', width / 2, height / 2 + 30);
        
        return canvas.toBuffer();
    }
    
    if (playerData.notFound) {
        // اللاعب لم يدخل السيرفر من قبل
        ctx.fillStyle = '#0f0f1b';
        ctx.fillRect(0, 0, width, height);
        ...
        ctx.font = 'bold 36px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('🔒 Never Joined', width / 2, height / 2 - 20);
        
        ctx.fillStyle = 'rgba(255, 255, 255, 0.6)';
        ctx.font = '20px Arial';
        ctx.fillText('This player has never joined the server', width / 2, height / 2 + 30);
        
        return canvas.toBuffer();
    }
    
    if (playerData.neverJoinedServer) {
        // اللاعب موجود لكن لم يدخل السيرفر من قبل (world و ip كلاهما unknown)
        ctx.fillStyle = '#0f0f1b';
        ctx.fillRect(0, 0, width, height);
        ...
        ctx.font = 'bold 36px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('📍 No Server Data', width / 2, height / 2 - 20);
        
        ctx.fillStyle = 'rgba(255, 255, 255, 0.6)';
        ctx.font = '20px Arial';
        ctx.fillText('Player has never been on the server', width / 2, height / 2 + 30);
    ...
    // رسالة الحالة (Offline Status Message)
    if (playerData.customApi && !playerData.isOnline) {
        ctx.fillStyle = 'rgba(255, 107, 107, 0.7)';
        ctx.font = '16px Arial';
        ctx.fillText('(Player is currently offline)', infoX, 160);
    }
    ...
    ctx.beginPath();
    ctx.roundRect(infoX + 160, 165, 140, 38, 12);
    ctx.fill();
    
    ctx.fillStyle = accountTypeColor;
    ctx.font = 'bold 22px Arial';
    ctx.textAlign = 'center';
    ctx.fillText(accountTypeText, infoX + 230, 193);
    ctx.textAlign = 'left';
