...
async function generatePlayerCard(ign, template = 'glass', serverConfig = null) {
    ...
    if (!playerData.endpointOffline && !playerData.notFound && !playerData.neverJoinedServer && !playerData.isOnline) {
    ...
    if (!playerData.endpointOffline && !playerData.notFound && !playerData.neverJoinedServer) {
        ...
        if (playerData.isOnline) {
            ...
            stats.push({ label: "Status", value: playerData.isBanned ? "Banned" : "Offline", icon: "globe", color: playerData.isBanned ? "#8B0000" : "#FF6B6B" });
        }

        stats.forEach((stat, i) => drawStatBox(stat.label, stat.value, stat.icon, stat.color, i));
    } else if (playerData.endpointOffline) {
        drawStatBox("Server Status", "ENDPOINT OFFLINE", "globe", "#FFA500", 0);
    } else {
