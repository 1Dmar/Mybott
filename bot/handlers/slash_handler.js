...
module.exports = async (client) => {
  try {
    ...
    if (!process.env.BOT1_1_TOKEN) {
      ...
      console.error("💡 Solution: Add BOT1_1_TOKEN to your Railway environment variables.");
      return;
    }
          ...
          for (const cmd of commands) {
            try {
              ...
              if (command?.name && command?.description && command?.run) {
                ...
                console.warn(`⚠️ Command ${cmd} is missing required fields (name, description, or run)`);
              }
            } catch (cmdError) {
              console.error(`❌ Error loading command ${cmd}:`, cmdError.message);
            }
          }
        });
      ...
      try {
        ...
        if (GUILD_ID) {
          try {
            ...
            if (guildError.code === 50001 || 
              ...
              console.error("");
              console.error("╔════════════════════════════════════════════════════════════════╗");
              console.error("║  🔒 MISSING ACCESS ERROR - ACTION REQUIRED!                    ║");
              console.error("╠════════════════════════════════════════════════════════════════╣");
              ...
              console.error("╚════════════════════════════════════════════════════════════════╝");
              console.error("");
              console.error(`💡 Quick invite link: https://discord.com/oauth2/authorize?client_id=${clientId}&scope=bot+applications.commands&permissions=8`);
              console.error("");
            }
          }
        }
        ...
        try {
          ...
          if (globalError.code === 50001 || 
            ...
            console.error("");
            console.error("╔════════════════════════════════════════════════════════════════╗");
            console.error("║  🔒 BOT TOKEN ERROR - ACTION REQUIRED!                         ║");
            console.error("╠════════════════════════════════════════════════════════════════╣");
