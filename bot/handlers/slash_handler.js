    ...
    client.once("ready", async () => {
      try {
        ...
        try {
          ...
          const guildErrorCode = guildError.code || guildError.status;
          if (guildErrorCode === 50001 || guildError.message?.includes('Unauthorized') || guildError.message?.includes('not authorized')) {
            console.warn("🔒 The bot is not authorized to register commands in this guild.");
            console.warn("💡 This usually means the bot was invited without the 'applications.commands' scope.");
          }
          ...
          try {
            ...
            const errorCode = globalError.code || globalError.status;
            
            if (errorCode === 50001 || globalError.message?.includes('Unauthorized') || globalError.message?.includes('not authorized')) {
              ...
              console.error("📋 Invite URL format: https://discord.com/oauth2/authorize?client_id=YOUR_CLIENT_ID&scope=bot+applications.commands&permissions=PERMISSIONS");
            ...
            } else if (errorCode === 50035 || globalError.message?.includes('Invalid Form Body')) {
              console.error("📝 Invalid command format detected");
              console.error("💡 Solution: Check that all command names are lowercase and descriptions are valid");
            } else {
              console.error("⚠️ Unknown error during registration");
              console.error("💡 Try reinviting the bot with the 'applications.commands' scope");
            }
            
            console.log("📝 Note: Commands are still loaded in memory and will work if previously registered.");
          }
        }
